...
#ifndef NATIVE_LIBLICENSE_SRC_SECURITY_MCBDESIMPL_HPP_
#define NATIVE_LIBLICENSE_SRC_SECURITY_MCBDESIMPL_HPP_
...
public:
    ...
    */
    ...
    inline unsigned int McbGetCryptogramSize() const
        { return m_cbResult; }

    inline unsigned int McbGetPlainTextSize() const
        { return m_cbResult; }
    ...
    *************************************************************************
    */
	inline unsigned int McbCalcCryptogramSize(unsigned int cbPlainText)
    {
    ...
    *************************************************************************
    */
	unsigned int McbCalcPlainTextSize(unsigned int cbCryptogram)
    {
    ...
    */
    ...
    {
        unsigned int cbPlainText = strlen(lpszPlainText);
...
protected:
    ...
    *************************************************************************
    */
	inline unsigned int McbGetOutputSize() const
        { return m_cbOutput; }
    ...
    *************************************************************************
    */
	inline void McbSetIterations(unsigned int nIterations)
    {
    ...
    *************************************************************************
    */
	inline unsigned int McbGetIterations() const { return m_nIterations; }
    ...
    *************************************************************************
    */
	inline void McbResizeOutput(unsigned int cb)
    {
    ...
    *************************************************************************
    */
	bool McbDoDES(const unsigned char * lpIn, unsigned int cbIn,
        bool bEncrypt);
    ...
    */
    ...
    unsigned int m_cbOutput;           // size of output buffer
    unsigned int m_cbResult;           // size of the result.
    ...
    unsigned int m_nIterations;        // single or triple des.
 ...
 ****************************************************************************
 */
template <int j> void McbDESImpl<j>::McbCreateSubKeys(
    const unsigned char Key[8], unsigned char SubKeys[16][6])
{
   ...
   /*
    *************************************************************************
    * Obtain first pair in int format (C0 in [0][0], D0 in [0][1])
    *************************************************************************
    */
    ...
    */
    for (n=0; n < 16; n++)
    {
       /*
        *********************************************************************
        * convert int pair into byte pair
        *********************************************************************
        */
 ...
 ****************************************************************************
 */
template <int j> void McbDESImpl<j>::McbMapThroughSBox(
 ...
 ****************************************************************************
 */
template <int j> void McbDESImpl<j>::McbEncode64Bits(
    ...
    */
	if (nEncrypt != 0)
    {
        for(n = 1; n < 17; n++)
        {
            ...
            *****************************************************************
            */
            *(unsigned int*)(data[n]) = *(unsigned int*)(&(data[n-1][4]));
 ...
 *                                decrypt.
 * @param       int cbIn		- (in) number of bytes in the in buffer
 ...
 ****************************************************************************
 */
template <int j> bool McbDESImpl<j>::McbDoDES(const unsigned char * lpIn,
    unsigned int cbIn, bool bEncrypt)
{
/*    boolshortcoming with using this
 * padding method is that an extra eight bytes will be added to the
 * cryptogram when encrypted if the size of the original data ends is a
 * multiple of eight.  For further details see the PKCS#5 comments near the
 * start of this function. </P>
 ...
 */
    ...
    unsigned int cbBlock, cbOut, cbCrypt, n;
 //	unsigned int cbBlock, cbOut, cbCrypt;
...
std::basic_string<char>encrypt_des(char *ori_pwd, char *key)
{
		std::basic_string<char> strPlainText;
    	std::basic_string<char> strCryptogram;

        unsigned char *lpKey1=(unsigned char *) key;
        /*
        *****************************************************************
        * Set the mode to single DES (triple DES is the default) and set
        * the encryption key (which must be eight bytes int).
        *****************************************************************
        */
		...
		des.McbSetDES();
		des.McbSetCBC(false);
		des.McbSetPadding(false);
        des.McbSetKey1(lpKey1);


       /*
        *****************************************************************
        * Calculate the size of the cryptogram - the macros defined at
        * the top of the header file can also be used for this purpose
        * for fixed size structures.
        *****************************************************************
        */
		//strPlainText = "Martyn was ere!";
        strPlainText = (char *) ori_pwd;
        unsigned int cbCryptogram =
       ...
       /*
        *****************************************************************
        * Perform the encryption - if this returns true then the results
        * will be placed in strCryptogram.  We also need to update the
        * size.
        *****************************************************************
        */
        ...
        /*
		if(strCryptogram.size()!=32)
			{
				printf("size != 32 %d\n",strCryptogram.size());
				getchar();
			}
			*/
		//memcpy(out_pwd, strCryptogram.data(),strCryptogram.size());

		return strCryptogram;
}


int decrypt_des(std::basic_string<char> strCryptogram,char *key,char *ori_pwd)
{
		//std::basic_string<char> strPlainText;
    	//std::basic_string<char> strCryptogram;
		...
		des.McbSetOutputBuffer((unsigned char*)ori_pwd,18);

        des.McbDecrypt((unsigned char*)strCryptogram.data(),
                strCryptogram.size());

        //printf("Decryption (%d) bytes: %s, \n",
          //      des.McbGetPlainTextSize(), des.McbGetPlainText());

		strcpy((char *)ori_pwd,(char *)des.McbGetPlainText());
		//memcpy(ori_pwd, strCryptogram.data(),strCryptogram.size());
		ori_pwd[18]='\0';
		return 0;
}
 ...
 ****************************************************************************
 */
//yh typedef McbDESImpl<0> McbDES;
//template <int n>

//#define McbDESImpl<0> McbDES
...
#endif /* NATIVE_LIBLICENSE_SRC_SECURITY_MCBDESIMPL_HPP_ */
